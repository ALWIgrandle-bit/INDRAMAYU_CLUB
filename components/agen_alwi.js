...
// ═══════════════════════════════════════════════════════════════════
// AGEN ALWI - NAVIGASI & SUNTIK MP3 TERINTEGRASI (TERMUX)
// Edisi: Grandew Cache & Audio Injector Aktif
// ═══════════════════════════════════════════════════════════════════
...
const AGEN_ALWI_CONFIG = {
  ...
  version: '2.0.6_grandew' // Mengunci versi cache agar selalu segar
};
...
const MENU_LAYANAN = {
  'nur1': {
    ...
    pages: [
      { key: 'media', label: 'Media Utama', url: 'nur1_media.html' }
    ]
  },
  'nur2': {
  ...
  'nur3': {
    ...
    desc: 'Dashboard Keuangan Pagi (Kas 5% & Voucher)',
    pages: [
      { key: 'keuangan', label: 'Keuangan Pagi', url: 'nur3_keuangan_pagi.html' }
    ]
  },
  'nur4': {
    ...
    pages: [
      { key: 'aset', label: 'Aset Digital', url: 'nur4_aset_digital.html' }
    ]
  },
  'nur5': {
    ...
    pages: [
      { key: 'menu', label: 'Menu Utama', url: 'nur5_menu_utama.html' }
    ]
  },
  'nur6': {
    label: '💰 Nur 6 - BANK_GAME',
    desc: 'Vault Kas & Admin Keuangan (folder terpisah)',
    pages: [
      { key: 'adm', label: 'Folder Admin Keuangan', url: '03_ADM_KEUANGAN/' }
    ]
  },
  'nur7': {
  ...
  'nur8': {
    ...
    pages: [
      { key: 'emulator', label: 'Emulator', url: 'nur8_emulator.html' }
    ]
  },
  'nur9': {
    label: '🎮 Nur 9 - GAME_INDRAMAYU',
    desc: 'Belum tersedia di server (file belum dibuat)',
    pages: []
  },
  'nur10': {
    ...
    pages: [
      { key: 'kamus', label: 'Kamus Alwi', url: 'nur10_kamus_alwi.html' }
    ]
  }
};
...
// 🔊 FITUR SUNTIK MP3 UTAMA (Global Player)
function suntikAudioSistem(urlMp3) {
  try {
    let oldAudio = document.getElementById('audio-suntik-grandew');
    if (oldAudio) oldAudio.remove();

    const audio = new Audio(urlMp3);
    audio.id = 'audio-suntik-grandew';
    audio.autoplay = true;
    audio.volume = 0.85;
    document.body.appendChild(audio);
    console.log(`🔊 MP3 Disuntikkan: ${urlMp3}`);
  } catch (e) {
    console.log('Suntikan audio gagal diblokir browser, butuh interaksi user.');
  }
}
...
function playAzanSound() {
  ...
  // Coba putar MP3 Azan fisik jika ada, jika tidak ada fallback ke melodi Beep
  suntikAudioSistem('assets/audio/azan.mp3'); 
}
...
function renderAgenAlwiBot() {
    ...
    <div id="agenAlwiContainer" style="position: fixed; bottom: 20px; right: 20px; z-index: 9998; font-family: 'Inter', sans-serif;">
      ...
      <div id="agenAlwiWindow" style="display: none; position: absolute; bottom: 90px; right: 0; width: 380px; height: 500px; background: linear-gradient(180deg, #06140b 0%, #0a2e1c 100%); border: 2px solid #c9a84c; border-radius: 16px; box-shadow: 0 8px 32px rgba(0,0,0,0.7); flex-direction: column; z-index: 9999; animation: slideUp 0.4s ease;">
        <div style="background: linear-gradient(135deg, #06140b, #0a2e1c); border-bottom: 2px solid #c9a84c; padding: 14px 16px; display: flex; justify-content: space-between; align-items: center; border-radius: 14px 14px 0 0;">
          <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div>
              ...
              <div style="font-size: 10px; color: #c9a84c;">Grandew Navigasi v${AGEN_ALWI_CONFIG.version}</div>
            </div>
          </div>
        ...
        <div style="border-top: 1px solid #c9a84c; padding: 12px; display: flex; gap: 8px;">
          <input type="text" id="agenAlwiInput" placeholder="Ketik perintah (ex: nur3, mp3 [nama], menu)..." onkeypress="if(event.key==='Enter') kirimPesanKeAlwi()" style="flex: 1; background: #06140b; border: 1px solid #c9a84c; border-radius: 8px; padding: 10px 12px; color: #ffd700; font-size: 12px; outline: none;" />
  ...
  setTimeout(() => {
    tambahPesanAlwi('Assalamu\'alaikum! Saya Agen Alwi Grandew.\n\nSistem Cache dikunci aman. Navigasi 10 Nur sinkron file asli.\n\nKetik "menu" untuk daftar, atau ketik "mp3 nama_file" untuk suntik audio.');
  }, 500);
}
...
function tambahPesanAlwi(teks, dariUser = false) {
  ...
  if (!dariUser) bicaraGoogleTTS(teks, 'id');
}
...
function prosesKatalkunci(teks) {
  if (teks.includes('menu') || teks.includes('layanan')) {
    return 'Daftar Ruangan Nur:\n\n' +
      Object.values(MENU_LAYANAN).map(m => `${m.label}`).join('\n') +
      '\n\nKetik kode Nur (mis. "nur2") untuk masuk / lihat sub-halaman.';
  }

  // Handle Perintah Suntik MP3 via Chat Bot
  if (teks.startsWith('mp3 ')) {
    let namaAudio = teks.replace('mp3 ', '').trim();
    let urlTarget = `assets/audio/${namaAudio}.mp3`;
    suntikAudioSistem(urlTarget);
    return `🔊 Perintah diterima! Menyuntikkan audio: [${namaAudio}.mp3] langsung ke panggung utama Indramayu Club.`;
  }

  for (const [key, menu] of Object.entries(MENU_LAYANAN)) {
    if (!teks.includes(key)) continue;

    if (!menu.pages || menu.pages.length === 0) {
      return `${menu.label} belum memiliki halaman aktif di server (file belum dibuat).`;
    }

    if (menu.pages.length === 1) {
      const target = menu.pages[0].url;
      setTimeout(() => { window.location.href = target; }, 1200);
      return `Mengarahkan ke ${menu.label} (File: ${target})...`;
    }

    const subMatch = menu.pages.find(p => teks.includes(p.key));
    if (subMatch) {
      setTimeout(() => { window.location.href = subMatch.url; }, 1200);
      return `Mengarahkan ke ${menu.label} - ${subMatch.label} (File: ${subMatch.url})...`;
    }

    return `${menu.label} punya beberapa halaman:\n` +
      menu.pages.map(p => `• ${p.label} → ketik "${key} ${p.key}"`).join('\n');
  }

  return 'Perintah tidak dikenali. Ketik "menu" untuk bantuan, atau "mp3 [nama]" untuk memutar audio.';
}
  ...
  setInterval(() => {
    ...
    for (const [key, shalat] of Object.entries(JADWAL_AZAN)) {
      if (jamSekarang === shalat.waktu) {
        playAzanSound();
        tambahPesanAlwi(`Waktu shalat ${shalat.nama} untuk wilayah Indramayu tiba. Mari rehat sejenak.`, false);
        break;
      }
    }
  }, 60000);
